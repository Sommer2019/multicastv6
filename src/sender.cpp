/* src/sender.cpp C++ IPv6 multicast sender (roundsend) with stream_id. Header per packet (12 bytes): 4 bytes stream_id (BE) 4 bytes sequence (BE) 4 bytes flags (BE) - bit0 = final */ 
#include <arpa/inet.h> 
#include <net/if.h> 
...
static constexpr size_t PAYLOAD_SIZE = 1200; static constexpr size_t HDR_LEN = 12; static constexpr uint32_t FLAG_FINAL = 1;

volatile sig_atomic_t g_interrupted = 0; void sigint_handler(int) { g_interrupted = 1; }

int main(int argc, char** argv) { std::string iface; std::string addr = "ff3e::1"; int port = 12345; std::string filename; int pps = 0; uint32_t stream_id = 1;for (int i = 1; i < argc; ++i) {
    std::string a(argv[i]);
...
while (!infile.eof() && !g_interrupted) {
    ...
    uint32_t flags_be = htonl(0);
    if (infile.eof()) flags_be = htonl(FLAG_FINAL);

    std::memcpy(buf.data(), &sid_be, 4);
    std::memcpy(buf.data()+4, &seq_be, 4);
    std::memcpy(buf.data()+8, &flags_be, 4);

    size_t tosend = HDR_LEN + (size_t)n;
